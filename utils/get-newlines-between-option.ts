...
interface GetNewlinesBetweenOptionParameters {
 ...
 * The result is based on the global `newlinesBetween` option and the custom groups,
 * which can override the global option.
 * - If the two nodes are in the same custom group, the `newlinesInside` option of the group is used.
 * - If the two nodes are in different custom groups, the `newlinesAbove` option of the previous group
 * and the `newlinesBelow` option of the next group are used.
 ...
 * @returns {'ignore' | 'always' | 'never'} - The `newlinesBetween` option to use
 */
export let getNewlinesBetweenOption = ({
  ...
  if (nodeCustomGroup && nodeCustomGroup === nextNodeCustomGroup) {
...
let getGlobalNewlinesBetweenOption = ({
  ...
  return nodeGroupNumber === nextNodeGroupNumber ? 'never' : 'global_always'
}
